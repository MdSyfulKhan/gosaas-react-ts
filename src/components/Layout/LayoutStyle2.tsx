...
interface LayoutStyle2Props {
	className?: string;
}

const LayoutStyle2: React.FC<LayoutStyle2Props> = ({ className = "" }) => {
	return (
		<div className={`cs_site_wrapper ${className}`}>
			<HeaderStyle2 logo="/img/logo.svg" logoUrl="/" />
			<main className="cs_main_content">
				<Outlet />
			</main>
			<Footer data={footerData} styleVariant="cs_type_1" />
		</div>
	);
};
